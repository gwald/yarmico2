...
#include "yar_cfg.h" // build def's changed before different builds, easier then setting make param's PITA!
...
#define YARMICO_SCREEN_WIDTH_MID  160

#define YARMICO_SCREEN_HEIGHT_MID 120
...
#define YARMICO_SCREEN_WIDTH_MID  120
...
#define YARMICO_SCREEN_WIDTH_MID  80

#define YARMICO_SCREEN_HEIGHT_MID 60
...
#define YARMICO_SCREEN_WIDTH_MID  60
...
#ifndef YARMICO_USE_TILEMAP

// no tilemap used

#define YARMICO_USE_TILEMAP FALSE
...
#define LOG_AUDIO // yarmico_debugf

#define LOG_TEXT // yarmico_debugf
...
#ifndef YARMICO_MAX_TILEMAP_WIDTH_SIZE

#define YARMICO_MAX_TILEMAP_WIDTH_SIZE 32
...
#ifndef YARMICO_MAX_TILEMAP_HEIGHT_SIZE

#define YARMICO_MAX_TILEMAP_HEIGHT_SIZE 32
...
// https://github.com/martincohen/Punity/blob/def395699e24670ae92d438f5b6b5e60aeec961c/punity.h#L814
...

{
	...
	u8 tile_cell_u; //x or row

	u8 tile_cell_v; //y or column
...
// populated via tilestudio menu: code -> project lists

typedef struct

{

	u8 tpage; //image texture page

//	u8 number_of_levels; // total maps, starting from zero

	u32 map_width; // all maps have the same width height

	u32 map_height;

	u8 tile_size; // square cells - no rectangles



	u32 current_level; // start level, set to zero



	//extracted from map on each level load

	u32 start_cell_pos_X, start_cell_pos_Y;





	yarmico_tile_data_t* map_data;

	yarmico_tile_data_t* cur_map_data;



	yarmico_RGB_t background_colors;
...
INSTANCE_API u8 g_yarmico_background_color_red, g_yarmico_background_color_green, g_yarmico_background_color_blue;
...
#if HARDWARE_TARGET == HARDWARE_TARGET_NET_YAROZE
#define yarmico_debugf printf
#else
void yarmico_debugf(char *format, ...);
#endif
...
INSTANCE_API s32 g_yarmico_tilemap_camera_x;

INSTANCE_API s32 g_yarmico_tilemap_camera_y;





// created via tilestudio tsd script
...
static inline u32 yarmico_tilemap_sprite(u8 tpage, u32 texture_u, u32 texture_v, u32 texture_w, u32 texture_h, s32 posX, s32 posY, s32 scale , u8 h_flip, u8 v_flip)

{
	...
	xs =  YARMICO_SCREEN_WIDTH_MID + posX -  g_yarmico_tilemap_camera_x;

	ys =  YARMICO_SCREEN_HEIGHT_MID + posY -  g_yarmico_tilemap_camera_y;



	if(xs > 0 && ys > 0 && xs < YARMICO_SCREEN_WIDTH && ys < YARMICO_SCREEN_HEIGHT)

	{

		yarmico_sprite(tpage,      texture_u,texture_v,      texture_w, texture_h, xs, ys  ,scale, h_flip,v_flip );//g_yarmico_collision_test_x, g_yarmico_collision_test_y, ONE,g_player.direction , 0);
...
static inline u32 yarmico_tilemap_rectangle(u8 r, u8 g, u8 b, u32 posX, u32 posY, u32 w, u32 h )

{
...

	{
		...
		yarmico_rectangle(r,g,b,  xs, ys,w, h );//g_yarmico_collision_test_x, g_yarmico_collision_test_y, ONE,g_player.direction , 0);
...
static inline u32 yarmico_tilemap_in_screen(s32 posX, s32 posY)
...
void yarmico_tilemap_init(yarmico_tile_data_t* levels);
...
typedef struct  {
	...
	u32 screen_x_fixed, screen_y_fixed; // screen space translation
...
INSTANCE_API u32 g_yarmico_collision_test_w;

INSTANCE_API s32 g_yarmico_collision_test_y;
INSTANCE_API u32 g_yarmico_collision_test_h;



INSTANCE_API yarmico_rect_t  g_yarmico_screen_rect;
...
#define LOG_COLLISION   //yarmico_debugf



//#define  yarmico_setup_collision_rec(_rect)  yarmico_debugf("."); g_yarmico_collision_test_x= _rect.x>0?_rect.x:0; g_yarmico_collision_test_y= _rect.y>0?_rect.y:0; g_yarmico_collision_test_w= g_yarmico_collision_test_x+_rect.w; g_yarmico_collision_test_h=g_yarmico_collision_test_y+_rect.h;


// yarmico_debugf("yarmico_setup_collision %d,%d %d,%d\n", (_DEFXX) , (_DEFYY), (_DEFWW), (_DEFHH) );
// yarmico_debugf("yarmico_setup_collision %d,%d %d,%d\n", g_yarmico_collision_test_x, g_yarmico_collision_test_y, g_yarmico_collision_test_w, g_yarmico_collision_test_h );
#define  yarmico_setup_collision( _DEFXX, _DEFYY, _DEFWW, _DEFHH)  { g_yarmico_collision_test_x= (_DEFXX)>1?(_DEFXX):0; g_yarmico_collision_test_y= (_DEFYY)>1?_DEFYY:0; g_yarmico_collision_test_w= g_yarmico_collision_test_x+_DEFWW; g_yarmico_collision_test_h=g_yarmico_collision_test_y+_DEFHH; }

//#define  yarmico_setup_collision( _DEFXX, _DEFYY, _DEFWW, _DEFHH)  {  yarmico_debugf("yarmico_setup_collision %d,%d %d,%d\n", (_DEFXX) , (_DEFYY), (_DEFWW), (_DEFHH) ); g_yarmico_collision_test_x= (_DEFXX)>1?(_DEFXX):0; g_yarmico_collision_test_y= (_DEFYY)>1?_DEFYY:0; g_yarmico_collision_test_w= g_yarmico_collision_test_x+_DEFWW; g_yarmico_collision_test_h=g_yarmico_collision_test_y+_DEFHH; yarmico_debugf("yarmico_setup_collision %d,%d %d,%d\n", g_yarmico_collision_test_x, g_yarmico_collision_test_y, g_yarmico_collision_test_w, g_yarmico_collision_test_h );}
// #define  yarmico_setup_collision( _DEFXX, _DEFYY, _DEFWW, _DEFHH)  {yarmico_debugf("yarmico_setup_collision %d,%d %d,%d\n", (_DEFXX) , (_DEFYY), (_DEFWW), (_DEFHH) ); g_yarmico_collision_test_x= (_DEFXX); g_yarmico_collision_test_y= _DEFYY; g_yarmico_collision_test_w= g_yarmico_collision_test_x+_DEFWW; g_yarmico_collision_test_h=g_yarmico_collision_test_y+_DEFHH;}
...
static inline u32  yarmico_rect_collision_test(yarmico_rect_t *b) //  screen space only, it can be negative because of the camera
{
...
#if 0 //check is assumed to be screen space only
	...
	if(bBottom > YARMICO_SCREEN_HEIGHT)

	{

		LOG_COLLISION("if(bBottom > YARMICO_SCREEN_HEIGHT) %d \n", bBottom);
	...
	if(bRight > YARMICO_SCREEN_WIDTH)

	{

		LOG_COLLISION("if(bRight > YARMICO_SCREEN_WIDTH)%d \n", bRight);
	...
	ret =	YARMICO_OVERLAP_TEST(g_yarmico_collision_test_y, g_yarmico_collision_test_h,  bTop,bBottom) && YARMICO_OVERLAP_TEST(g_yarmico_collision_test_x, g_yarmico_collision_test_w, bLeft, bRight);

	if(ret)
	{
		LOG_COLLISION("yarmico_rect_collision_test XY/WH %d,%d %d,%d \n", g_yarmico_collision_test_x, g_yarmico_collision_test_y, g_yarmico_collision_test_w, g_yarmico_collision_test_h);
		LOG_COLLISION("yarmico_rect_collision_test LR/TB %d,%d %d,%d \n", bLeft, bRight, bTop, bBottom);


		LOG_COLLISION("yarmico_rect_collision_test %d X %d,%d %d,%d \n", YARMICO_OVERLAP_TEST(g_yarmico_collision_test_x, g_yarmico_collision_test_w, bLeft, bRight), g_yarmico_collision_test_x,g_yarmico_collision_test_w, bLeft, bRight);
		LOG_COLLISION("yarmico_rect_collision_test %d Y %d,%d %d,%d \n",YARMICO_OVERLAP_TEST(g_yarmico_collision_test_y, g_yarmico_collision_test_h,  bTop,bBottom), g_yarmico_collision_test_y, g_yarmico_collision_test_h,  bTop, bBottom);

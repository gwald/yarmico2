...
void yarmico_sprite(u8 tpage, u32 texture_u, u32 texture_v, u32 texture_w, u32 texture_h, s32 posX, s32 posY, s32 scale , u8 h_flip, u8 v_flip)
{
	...
	tim->x = posX ;
	tim->y = posY;
	tim->x +=NATIVE_SCREEN_WIDTH_BORDER_START_240X240_256X240;
	tim->y +=NATIVE_SCREEN_HEIGHT_BORDER_START_240X240_256X240;
	...
	//NOTE: the following is probably a bug in the NY GsSortSprite functions?
	// IT'S A PITA!
	// Trial and error waste of time to get magic numbers, they are for 8x8 cells in a TPage sized TIM - may or may not work with different cells/TIM's
	...
	if(h_flip)
	{
		...
		if(scale_flip ==0  )
		{
			// TODO: hack,  don't know why this is needed?
	...
	if(v_flip)
	{
		...
		{
			//nopsx		tim->scalex -=512;
			//nopsx		tim->scaley -=256;
...
void yarmico_text_init(void)
{
	...
	i = FntOpen(HALF_WIDTH-3*16,HALF_HEIGHT+16, NATIVE_SCREEN_WIDTH,2*16,0,160);
	LOG_DEL("FntOpen %d\n",i);
	//
...
yarmico_tile_data_t* yarmico_tilemap_get_cell(u32 x, u32 y)
{
	...
	cell = g_yarmico_maps.cur_map_data;// start of cur map
	cell += y * g_yarmico_maps.map_width;  // move down to correct y row
	cell += x;// move across to right x col

	LOG_TILEMAP( "\n yarmico_tilemap_get_cell  %d,%d = %d  U:%d V:%d %p\n", x,y, x+(y * g_yarmico_maps.map_width), cell->tile_cell_u, cell->tile_cell_v, cell);

	return cell;

}

// link header to gmap pointer
void yarmico_tilemap_init(yarmico_tile_data_t* level_map_address)
{
	u32 i, size, cell, x, y, mapsize;
	yarmico_tile_data_t* c;


	// LOG_TILEMAP( "\n yarmico_tilemap_init  levels %p \n", level_map_address);


	//	levels += 4;  //; 4byte header "MAP" + u8
	g_yarmico_maps.map_data = level_map_address;
	g_yarmico_maps.cur_map_data = (yarmico_tile_data_t*) NULL;
	mapsize = g_yarmico_maps.map_width * g_yarmico_maps.map_height ;
	size = g_yarmico_maps.tile_size;

	g_yarmico_maps.start_cell_pos_X *= size; // expand size to pixel location
	g_yarmico_maps.start_cell_pos_Y *= size; // expand size to pixel location

	//expand tilemap cell number to UV TPage coords
	// removed multiple maps for(maps=0 ; maps<  g_yarmico_maps.number_of_levels ; maps++)
	{
		for(i=0 ; i< mapsize ; i++)
		{

			c = level_map_address+i;

			//	LOG_TILEMAP( "\n yarmico_tilemap_init  c %p \n", c);

			cell = c->tile_cell_v;


			y = i/g_yarmico_maps.map_width;
			x = i-y*g_yarmico_maps.map_width;

			if( c->tile_cell_u != x)
				printf("\n FAILED CHECK map#:%d cell#:%d c->tile_cell_u:%d  <> x:%d",0, i, c->tile_cell_u, x);




			c->tile_cell_u = 0; // clear # check

			if(cell > 0) // not empty
			{

				// expand cell to UV
				y = cell  >> 4; // div by 16; //16 cells per row
				x = cell - (y * 16); // modulation, ie cell % 16 // 16 cells per col , the rest is x across





				c->tile_cell_u = (u8) (x *  size); //cell to pixel coords
				c->tile_cell_v = (u8) (y *  size); //cell to pixel coords
				//	LOG_TILEMAP( "\n map %d i:%d cell %d %d-%d c->tile_mapcode %d",maps, i, cell, c->tile_cell_u,c->tile_cell_v, c->tile_mapcode);
			}
		...
		// tilemap_next_level(1);

		level_map_address +=  mapsize;
		//levels += 4;  //; 4byte header "MAP" + u8
	}


	//g_yarmico_maps.cur_map_data  += ( sizeof(yarmico_tile_data_t)* mapsize ) +4;
...
{
	u32 	levels_stride , level_size ;
	u32 level;

	u32 i, maps, size, cell, x, y, mapsize;
	yarmico_tile_data_t* c;

	level =g_yarmico_maps.current_level;
	level_size = levels_stride =  (g_yarmico_maps.map_height * g_yarmico_maps.map_width);

	levels_stride *= level;


	LOG_TILEMAP( "\n yarmico_tilemap_get_next_level  levels_stride %p \n", levels_stride);



// wasteful not resusing the original RAM, but level gets overwriten, so has to be copied
	if(g_yarmico_maps.cur_map_data == NULL) // assign memory
	{
		g_yarmico_maps.cur_map_data = (yarmico_tile_data_t*) malloc(level_size * sizeof(yarmico_tile_data_t)); // assign single level map size
	}
	memcpy(g_yarmico_maps.cur_map_data, g_yarmico_maps.map_data + levels_stride, level_size * sizeof(yarmico_tile_data_t));
...
#endif //#if YARMICO_USE_TILEMAP == TRUE
...
void init(void)
{
	...
	// link header to gmap pointer
	yarmico_tilemap_init( (yarmico_tile_data_t *) LEVELS_DATA);
	// called from game.c yarmico_tilemap_get_next_level();// init level zero
#endif
...
#define GPU_GP1_(x)	*((volatile u32*)(0x1f801814 + (x<<4)))
	...
	// use display, pad and unknown RAM data as rand seed?
	{
		...
		g_yarmico_game_time = PLAYSTATION_REGION_CHECK();
		LOG_MAIN("\n( *((volatile u32 *)GPUSTAT_REG) ) %X\n", g_yarmico_game_time);
		LOG_MAIN("\nReg_GPUSTAT %X\n", g_yarmico_game_time);

		if(g_yarmico_game_time) // if 1, it's PAL
		{
	...
	g_yarmico_game_time = ( *((volatile u32 *)GPUSTAT_REG) );
	g_yarmico_game_time +=((rand()& 0xFFFF) );
	printf("\nReg_GPUSTAT + rand %X\n", g_yarmico_game_time);
	srand(g_yarmico_game_time);
#endif
		...
		else
		{
			...
			if(g_yarmico_controller_special_buttons & YARMICO_CONTROLER_BUT_HOME_SELECT)
			{
				...
				else // use big borders
				{
					...
					if(NATIVE_SCREEN_WIDTH == 160) // 160 to 256 width
					{
						g_yarmico_lores_upscale[0] = 6552; //fixed point scale factor 160 x 1.6 = 256 so  1.6 x ONE = 6553.6 6552%8 is 0
						g_yarmico_lores_upscale[1] = 0;
						g_yarmico_lores_upscale[2] = 0;
					}
					else //120 to 256 width
...
#endif //#if NATIVE_SCREEN_HEIGHT == 120

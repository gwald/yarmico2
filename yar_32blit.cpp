...
void yarmico_debugf( char *format, ...) { return;}
#endif
...
#elif YARMICO_NATIVE_DISPLAY==YARMICO_NATIVE_DISPLAY_160x120 // vgaboard - TODO: logic should be better
	screen_posX += 20;
...
// no controller support? #ifdef __EMSCRIPTEN__
...
void render(uint32_t time)
{
	...
	screen.alpha = 255;
	screen.pen = blit::Pen(g_yarmico_background_color_red, g_yarmico_background_color_green, g_yarmico_background_color_blue);
...
u8*  yarmico_tilemap_get_next_level(void)
{
	...
	for(i=0 ; i< level_size; i++)
	{
		...
		c =g_yarmico_maps.cur_map_data+i;
		...
		if(cell > 0) // not empty
		{
			...
			if(0)// c->tile_cell_u != x)
			{
				//		printf("\n FAILED CHECK map#:%d cell#:%d c->tile_cell_u:%d  <> x:%d",maps, i, c->tile_cell_u, x);
				//		fflush(stdout);
				//		exit(0);
			}
...
void yarmico_tilemap_init(u8* level_map_address)
{
	u32 size;
	...
	g_yarmico_maps.map_data = (yarmico_tile_data_t*) level_map_address;
...
void init() {
	...
	yarmico_tilemap_init( (u8*) levels_bin);
	// run in game.c yarmico_tilemap_get_next_level();
